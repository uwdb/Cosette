...
(define (q2 tables) 
  (SELECT (VALS "c.name" "f1.flight_num" "f1.origin_city" "f1.dest_city" "f1.actual_time" "f2.flight_num" "f2.origin_city" "f2.dest_city" "f2.actual_time" (VAL-BINOP "f1.actual_time" + "f2.actual_time")) 
 FROM (JOIN (NAMED (RENAME (list-ref tables 3) "f1")) (JOIN (NAMED (RENAME (list-ref tables 3) "f2")) (JOIN (NAMED (RENAME (list-ref tables 2) "c")) (NAMED (RENAME (list-ref tables 0) "m"))))) 
 WHERE (AND (AND (AND (AND (AND (AND (AND (AND (AND (AND (AND (AND (AND (AND (BINOP "f1.year" = "f2.year") (BINOP "f1.month_id" = "f2.month_id")) (BINOP "f1.day_of_week_id" = "f2.day_of_week_id")) (BINOP "f1.carrier_id" = "f2.carrier_id")) (BINOP (VAL-BINOP "f1.actual_time" + "f2.actual_time") < 420)) (BINOP "f1.year" = "f2.year")) (BINOP "f1.day_of_month" = "f2.day_of_month")) (BINOP "f1.month_id" = "f2.month_id")) (BINOP "f1.year" = 2015)) (BINOP "f1.day_of_month" = 15)) (BINOP "f1.month_id" = "m.mid")) (BINOP "m.month" = str_july_)) (BINOP "f1.origin_city" = str_seattle_wa_)) (BINOP "f2.dest_city" = str_boston_ma_)) (BINOP "f1.dest_city" = "f2.origin_city")) GROUP-BY (list "f1.flight_num" "f2.flight_num") 
 HAVING (TRUE)))

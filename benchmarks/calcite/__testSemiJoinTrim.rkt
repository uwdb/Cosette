...
(define (q1 tables) 
  ...
  WHERE (AND (BINOP "emp.deptno" = "dept.deptno") (BINOP "emp.sal" > 100))) ["anyname" (list "empno" "ename" "job" "mgr" "hiredate" "sal" "comm" "deptno" "slacker")]))) ["s" (list "deptno" "name")]) (AS (NAMED (list-ref tables 1)) ["account"])) 
...
(define (q2 tables) 
  ...
  FROM (AS (NAMED (list-ref tables 1)) ["account"]) 

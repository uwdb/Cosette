...
(define (q1 tables) 
  (SELECT (VALS "t.ename" (VAL-UNOP aggr-min (val-column-ref "t.empno"))) 
...
(define (q2 tables) 
  (SELECT (VALS "t6.ename" (VAL-UNOP aggr-min (VAL-UNOP aggr-min (val-column-ref "emp2.empno")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.ename" (VAL-UNOP aggr-min (val-column-ref "emp1.empno"))) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp1")) 
 WHERE (TRUE) GROUP-BY (list "emp1.ename") 
 HAVING (TRUE)) (SELECT (VALS "emp2.ename" (VAL-UNOP aggr-min (val-column-ref "emp2.empno"))) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp2")) 
 WHERE (TRUE) GROUP-BY (list "emp2.ename") 
 HAVING (TRUE))) ["t6" (list "ename" "min_empno")]) 

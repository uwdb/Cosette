...
(define (q1 tables) 
  (SELECT (VALS "t1.deptno" "t1.job" (VAL-UNOP aggr-sum (val-column-ref "t1.u"))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp.empno" "emp.ename" "emp.job" "emp.mgr" "emp.hiredate" "emp.sal" "emp.comm" "emp.deptno" "emp.slacker" null) 
  FROM (NAMED (RENAME (list-ref tables 4) "emp")) 
  WHERE (TRUE)) (SELECT (VALS "emp0.empno" "emp0.ename" "emp0.job" "emp0.mgr" "emp0.hiredate" "emp0.sal" "emp0.comm" "emp0.deptno" "emp0.slacker" null) 
  FROM (NAMED (RENAME (list-ref tables 4) "emp0")) 
  WHERE (TRUE))) ["t1" (list "empno" "ename" "job" "mgr" "hiredate" "sal" "comm" "deptno" "slacker" "u")]) 
 WHERE (TRUE) GROUP-BY (list "t1.deptno" "t1.job") 
 HAVING (TRUE)))

(define (q2 tables) 
  (SELECT (VALS "t8.deptno" "t8.job" (VAL-UNOP aggr-sum (VAL-UNOP aggr-sum null))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.deptno" "emp1.job" (VAL-UNOP aggr-sum null)) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp1")) 
 WHERE (TRUE) GROUP-BY (list "emp1.deptno" "emp1.job") 
 HAVING (TRUE)) (SELECT (VALS "emp2.deptno" "emp2.job" (VAL-UNOP aggr-sum null)) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp2")) 
 WHERE (TRUE) GROUP-BY (list "emp2.deptno" "emp2.job") 
 HAVING (TRUE))) ["t8" (list "deptno" "job" "sum_null")]) 
 WHERE (TRUE) GROUP-BY (list "t8.deptno" "t8.job") 
 HAVING (TRUE)))

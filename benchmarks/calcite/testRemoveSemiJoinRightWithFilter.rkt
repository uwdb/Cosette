...
(define (q2 tables) 
  ...
  FROM (JOIN (AS (NAMED (list-ref tables 1)) ["emp1"]) (JOIN (AS (SELECT (VALS "dept0.deptno" "dept0.name") 
  FROM (AS (NAMED (list-ref tables 0)) ["dept0"]) 
  WHERE (BINOP "dept0.name" = str_foo_)) ["t1" (list "deptno" "name")]) (AS (NAMED (list-ref tables 1)) ["emp2"]))) 

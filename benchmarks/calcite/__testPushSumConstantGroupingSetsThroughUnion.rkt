...
(define (q2 tables) 
  (SELECT (VALS "t8.deptno" "t8.job" (VAL-UNOP aggr-sum (VAL-UNOP aggr-sum 3))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.deptno" "emp1.job" (VAL-UNOP aggr-sum 2)) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp1")) 
 WHERE (TRUE) GROUP-BY (list "emp1.deptno" "emp1.job") 
 HAVING (TRUE)) (SELECT (VALS "emp2.deptno" "emp2.job" (VAL-UNOP aggr-sum 3)) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp2")) 
 WHERE (TRUE) GROUP-BY (list "emp2.deptno" "emp2.job") 
 HAVING (TRUE))) ["t8" (list "deptno" "job" "sum_num2")]) 

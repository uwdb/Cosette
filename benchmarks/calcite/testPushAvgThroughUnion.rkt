...
(define (q1 tables) 
  (SELECT (VALS "t.ename" (VAL-BINOP (VAL-UNOP aggr-sum (val-column-ref "t.empno")) div_ (VAL-UNOP aggr-count (val-column-ref "t.empno")))) 
...
(define (q2 tables) 
  (SELECT (VALS "t4.ename" (VAL-BINOP (VAL-UNOP aggr-sum (val-column-ref "t4.empno")) div_ (VAL-UNOP aggr-count (val-column-ref "t4.empno")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.ename" "emp1.empno") 
  FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
  WHERE (TRUE)) (SELECT (VALS "emp2.ename" "emp2.empno") 
  FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
  WHERE (TRUE))) ["t4" (list "ename" "empno")]) 
 WHERE (TRUE) GROUP-BY (list "t4.ename") 
 HAVING (TRUE)))

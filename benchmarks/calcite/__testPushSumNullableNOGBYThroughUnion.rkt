...
(define (q1 tables) 
  (SELECT (VALS (VAL-UNOP aggr-sum (val-column-ref "t.mgr"))) 
...
(define (q2 tables) 
  (SELECT (VALS (VAL-UNOP aggr-sum (VAL-UNOP aggr-sum (val-column-ref "emp2.mgr")))) 
 FROM (AS (UNION-ALL (SELECT (VALS (VAL-UNOP aggr-sum (val-column-ref "emp1.mgr"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
 WHERE (TRUE) GROUP-BY (list) 
 HAVING (TRUE)) (SELECT (VALS (VAL-UNOP aggr-sum (val-column-ref "emp2.mgr"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
 WHERE (TRUE) GROUP-BY (list) 
 HAVING (TRUE))) ["t6" (list "sum_mgr")]) 
 WHERE (TRUE) GROUP-BY (list) 
 HAVING (TRUE)))

...
(define (q1 tables) 
  (SELECT (VALS "t.deptno" "t.job" (VAL-UNOP aggr-min (val-column-ref "t.empno"))) 
...
(define (q2 tables) 
  (SELECT (VALS "t6.deptno" "t6.job" (VAL-UNOP aggr-min (VAL-UNOP aggr-min (val-column-ref "emp2.empno")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.deptno" "emp1.job" (VAL-UNOP aggr-min (val-column-ref "emp1.empno"))) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp1")) 
 WHERE (TRUE) GROUP-BY (list "emp1.deptno" "emp1.job") 
 HAVING (TRUE)) (SELECT (VALS "emp2.deptno" "emp2.job" (VAL-UNOP aggr-min (val-column-ref "emp2.empno"))) 
 FROM (NAMED (RENAME (list-ref tables 4) "emp2")) 
 WHERE (TRUE) GROUP-BY (list "emp2.deptno" "emp2.job") 
 HAVING (TRUE))) ["t6" (list "deptno" "job" "min_empno")]) 

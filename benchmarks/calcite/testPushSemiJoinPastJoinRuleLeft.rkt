...
(define (q2 tables) 
  ...
  FROM (JOIN (AS (NAMED (list-ref tables 1)) ["emp1"]) (JOIN (AS (NAMED (list-ref tables 0)) ["dept0"]) (JOIN (AS (NAMED (list-ref tables 1)) ["emp2"]) (JOIN (AS (NAMED (list-ref tables 0)) ["dept1"]) (AS (NAMED (list-ref tables 1)) ["emp3"]))))) 

...
(define (q1 tables) 
  (SELECT (VALS "t.deptno" "t.job" (VAL-UNOP aggr-sum (val-column-ref "t.empno")) (VAL-UNOP aggr-count (val-column-ref "t.empno")) (VAL-UNOP aggr-min (val-column-ref "t.deptno")) (VAL-UNOP aggr-max (val-column-ref "t.empno"))) 
...
(define (q2 tables) 
  (SELECT (VALS "t6.deptno" "t6.job" (VAL-UNOP aggr-sum (VAL-UNOP aggr-sum (val-column-ref "emp2.empno"))) (VAL-UNOP aggr-sum (VAL-UNOP aggr-count (val-column-ref "t6.deptno"))) (VAL-UNOP aggr-min (VAL-UNOP aggr-min (val-column-ref "emp2.deptno"))) (VAL-UNOP aggr-max (VAL-UNOP aggr-max (val-column-ref "emp2.empno")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.deptno" "emp1.job" (VAL-UNOP aggr-sum (val-column-ref "emp1.empno")) (VAL-UNOP aggr-count (val-column-ref "emp1.empno")) (VAL-UNOP aggr-min (val-column-ref "emp1.deptno")) (VAL-UNOP aggr-max (val-column-ref "emp1.empno"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
 WHERE (TRUE) GROUP-BY (list "emp1.deptno" "emp1.job") 
 HAVING (TRUE)) (SELECT (VALS "emp2.deptno" "emp2.job" (VAL-UNOP aggr-sum (val-column-ref "emp2.empno")) (VAL-UNOP aggr-count (val-column-ref "emp2.empno")) (VAL-UNOP aggr-min (val-column-ref "emp2.deptno")) (VAL-UNOP aggr-max (val-column-ref "emp2.empno"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
 WHERE (TRUE) GROUP-BY (list "emp2.deptno" "emp2.job") 
 HAVING (TRUE))) ["t6" (list "deptno" "job" "sum_empno" "count_star" "min_deptno" "max_empno")]) 

...
(define (q1 tables) 
  (SELECT (VALS "t.ename" (VAL-UNOP aggr-count (val-column-ref "t.mgr"))) 
...
(define (q2 tables) 
  (SELECT (VALS "t6.ename" (VAL-UNOP aggr-sum (VAL-UNOP aggr-count (val-column-ref "emp2.mgr")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.ename" (VAL-UNOP aggr-count (val-column-ref "emp1.mgr"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
 WHERE (TRUE) GROUP-BY (list "emp1.ename") 
 HAVING (TRUE)) (SELECT (VALS "emp2.ename" (VAL-UNOP aggr-count (val-column-ref "emp2.mgr"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
 WHERE (TRUE) GROUP-BY (list "emp2.ename") 
 HAVING (TRUE))) ["t6" (list "ename" "count_mgr")]) 

...
(define (q2 tables) 
  (SELECT (VALS "t8.ename" (VAL-UNOP aggr-sum (VAL-UNOP aggr-sum null))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.ename" (VAL-UNOP aggr-sum null)) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
 WHERE (TRUE) GROUP-BY (list "emp1.ename") 
 HAVING (TRUE)) (SELECT (VALS "emp2.ename" (VAL-UNOP aggr-sum null)) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
 WHERE (TRUE) GROUP-BY (list "emp2.ename") 
 HAVING (TRUE))) ["t8" (list "ename" "sum_null")]) 

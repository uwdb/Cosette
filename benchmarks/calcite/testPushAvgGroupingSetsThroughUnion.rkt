...
(define (q1 tables) 
  (SELECT (VALS "t.deptno" "t.job" (VAL-BINOP (VAL-UNOP aggr-sum (val-column-ref "t.empno")) div_ (VAL-UNOP aggr-count (val-column-ref "t.empno")))) 
...
(define (q2 tables) 
  (SELECT (VALS "t4.deptno" "t4.job" (VAL-BINOP (VAL-UNOP aggr-sum (val-column-ref "t4.empno")) div_ (VAL-UNOP aggr-count (val-column-ref "t4.empno")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.deptno" "emp1.job" "emp1.empno") 
  FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
  WHERE (TRUE)) (SELECT (VALS "emp2.deptno" "emp2.job" "emp2.empno") 
  FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
  WHERE (TRUE))) ["t4" (list "deptno" "job" "empno")]) 
 WHERE (TRUE) GROUP-BY (list "t4.deptno" "t4.job") 
 HAVING (TRUE)))

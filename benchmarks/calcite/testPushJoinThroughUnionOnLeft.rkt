...
(define (q1 tables) 
  (SELECT (VALS "t.sal") 
  FROM (JOIN (AS (UNION-ALL (SELECT (VALS "emp.empno" "emp.ename" "emp.job" "emp.mgr" "emp.hiredate" "emp.comm" "emp.sal" "emp.deptno" "emp.slacker") 
  ...
  WHERE (TRUE))) ["t" (list "empno" "ename" "job" "mgr" "hiredate" "comm" "sal" "deptno" "slacker")]) (NAMED (RENAME (list-ref tables 4) "emp1"))) 
  WHERE (TRUE)))
...
(define (q2 tables) 
  ...
  FROM (JOIN (NAMED (RENAME (list-ref tables 4) "emp2")) (NAMED (RENAME (list-ref tables 4) "emp3"))) 
  ...
  FROM (JOIN (NAMED (RENAME (list-ref tables 4) "emp4")) (NAMED (RENAME (list-ref tables 4) "emp5"))) 

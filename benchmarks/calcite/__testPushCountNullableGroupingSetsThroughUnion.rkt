...
(define (q1 tables) 
  (SELECT (VALS "t.deptno" "t.job" (VAL-UNOP aggr-count (val-column-ref "t.mgr"))) 
...
(define (q2 tables) 
  (SELECT (VALS "t6.deptno" "t6.job" (VAL-UNOP aggr-sum (VAL-UNOP aggr-count (val-column-ref "emp2.mgr")))) 
 FROM (AS (UNION-ALL (SELECT (VALS "emp1.deptno" "emp1.job" (VAL-UNOP aggr-count (val-column-ref "emp1.mgr"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp1"]) 
 WHERE (TRUE) GROUP-BY (list "emp1.deptno" "emp1.job") 
 HAVING (TRUE)) (SELECT (VALS "emp2.deptno" "emp2.job" (VAL-UNOP aggr-count (val-column-ref "emp2.mgr"))) 
 FROM (AS (NAMED (list-ref tables 4)) ["emp2"]) 
 WHERE (TRUE) GROUP-BY (list "emp2.deptno" "emp2.job") 
 HAVING (TRUE))) ["t6" (list "deptno" "job" "count_mgr")]) 

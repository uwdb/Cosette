...
(SELECT (VALS "t2.cmte_id" (SUM "t2.transaction_amt") (COUNT "t2.cmte_id") "t2.cmte_nm")
                         ...
                         HAVING (TRUE)))
(define (q2 tables)
(AS (SELECT (VALS "t.cmte_id" (SUM "t.transaction_amt") (COUNT "t.cmte_id") "t.cmte_nm")
FROM (AS (LEFT-OUTER-JOIN (AS (NAMED (list-ref tables 0))
["indiv_sample_nyc" (list "cmte_id" "transaction_amt" "name")]) (AS (NAMED (list-ref tables 1))
["comm" (list "cmte_id" "cmte_nm" "cand_id")]) (BINOP "indiv_sample_nyc.cmte_id" = "comm.cmte_id"))
["t" (list "cmte_id" "transaction_amt" "name" "cmte_id_2" "cmte_nm" "cand_id")])
WHERE (AND (AND (TRUE) (TRUE)) (TRUE)) 
GROUP-BY (list "t.cmte_id" ) 
HAVING (TRUE))
["t_2" (list "cmte_id" "total_amount" "num_donations" "cmte_nm")]))

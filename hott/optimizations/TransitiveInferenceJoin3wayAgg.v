...
Module Optimization (T : Types) (S : Schemas T) (R : Relations T S)  (A : Aggregators T S). 
  ...
  Definition Rule: Type. 
    ...
    refine (⟦ Γ ⊢ (SELECT1 (e2p (constantExpr integer_1)) FROM1 (product ((SELECT  (combine' PLAIN(variable (right⋅emp_deptno)) (COUNT(variable (right⋅emp_slacker)))) FROM1  (table rel_emp) WHERE (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_7)) ) gt) GROUP BY  (right⋅emp_deptno))) (product (table rel_emp) (table rel_emp))) WHERE (and (equal (variable (right⋅left⋅left)) (variable (right⋅right⋅left⋅emp_deptno))) (equal (variable (right⋅right⋅left⋅emp_deptno)) (variable (right⋅right⋅right⋅emp_deptno))))) : _ ⟧ =  ⟦ Γ ⊢ (SELECT1 (e2p (constantExpr integer_1)) FROM1 (product ((SELECT  (combine' PLAIN(variable (right⋅emp_deptno)) (COUNT(variable (right⋅emp_slacker)))) FROM1  (table rel_emp) WHERE (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_7)) ) gt) GROUP BY  (right⋅emp_deptno))) (product ((SELECT * FROM1 (table rel_emp) WHERE (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_7)) ) gt))) ((SELECT * FROM1 (table rel_emp) WHERE (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_7)) ) gt))))) WHERE (and (equal (variable (right⋅left⋅left)) (variable (right⋅right⋅left⋅emp_deptno))) (equal (variable (right⋅right⋅left⋅emp_deptno)) (variable (right⋅right⋅right⋅emp_deptno))))) : _ ⟧). 
  Defined. 
  Arguments Rule /. 
  ...
  Lemma ruleStand: Rule. 
    ...
    + intros [[[h1 h2] h3] h4].
      ...
      destruct h2.
      destruct h3.
      destruct h4 as [t [[e1 e2] e3]].
      rewrite <- (path_universe_uncurried (equiv_path_prod _ _)) in e3.
      destruct e3 as [e3 e4].
      simpl in *.
      destruct e3.
      repeat constructor; try simp_solve.
      apply tr.
      refine (t; _).
      repeat constructor; try simp_solve.

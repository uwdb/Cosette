...
Module Optimization (T : Types) (S : Schemas T) (R : Relations T S)  (A : Aggregators T S). 
  ...
  Variable integer_9: constant int. 
  ...
  Definition Rule: Type. 
    ...
    refine (⟦ Γ ⊢ (SELECT1 (e2p (constantExpr integer_1)) FROM1 (product ((SELECT * FROM1 (table rel_emp) WHERE (or (or (equal (variable (right⋅emp_deptno)) (constantExpr integer_7)) (equal (variable (right⋅emp_deptno)) (constantExpr integer_9))) (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_10)) ) gt)))) (table rel_emp)) WHERE (equal (variable (right⋅left⋅emp_deptno)) (variable (right⋅right⋅emp_deptno)))) : _ ⟧ =  ⟦ Γ ⊢ (SELECT1 (e2p (constantExpr integer_1)) FROM1 (product ((SELECT * FROM1 (table rel_emp) WHERE (or (or (equal (variable (right⋅emp_deptno)) (constantExpr integer_7)) (equal (variable (right⋅emp_deptno)) (constantExpr integer_9))) (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_10)) ) gt)))) ((SELECT * FROM1 (table rel_emp) WHERE (or (or (equal (variable (right⋅emp_deptno)) (constantExpr integer_7)) (equal (variable (right⋅emp_deptno)) (constantExpr integer_9))) (castPred (combine (right⋅emp_deptno) (e2p (constantExpr integer_10)) ) gt))))) WHERE (equal (variable (right⋅left⋅emp_deptno)) (variable (right⋅right⋅emp_deptno)))) : _ ⟧). 
  Defined. 
  Arguments Rule /. 

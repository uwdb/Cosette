...
Module Optimization (T : Types) (S : Schemas T) (R : Relations T S)  (A : Aggregators T S). 
  ...
  Definition Rule: Type. 
    ...
    refine (⟦ Γ ⊢ (SELECT1 (right⋅left⋅emp_sal) FROM1 (product (table rel_emp) (((SELECT * FROM1 (table rel_emp) )) UNION ALL ((SELECT * FROM1 (table rel_emp) )))) ) : _ ⟧ =  ⟦ Γ ⊢ (SELECT1 (right⋅right⋅emp_sal) FROM1 (((SELECT * FROM1 (product (table rel_emp) (table rel_emp)) )) UNION ALL ((SELECT * FROM1 (product (table rel_emp) (table rel_emp)) ))) ) : _ ⟧). 
  Defined. 
  Arguments Rule /. 
